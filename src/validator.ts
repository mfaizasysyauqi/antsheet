...
const VALID_VALIDATION_TYPES = new Set<string>([
  "list",
  "number",
  "textLength",
  "date",
...
const VALID_CONDITIONAL_TYPES = new Set<string>([
  "highlight",
  "colorScale",
  "dataBar",
]);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentSection === "Meta") {
      if (line.startsWith("columnWidth:")) {
        ...
        if (isNaN(Number(val))) errors.push(`Line ${lineNum}: Invalid freezeRow '${val}'. Expected number.`);
      } else if (line.startsWith("freezeColumn:")) {
        // can be number or letter — no strict check needed
      } else if (line.startsWith("freeze:")) {
        // e.g. freeze: row=1, col=1
        ...
        // comma separated numbers, skip strict check
      } else if (line.startsWith("hiddenColumns:")) {
        // comma separated letters, skip strict check
      } else {
...
function validateCellLine(
  ...
  const valueRaw = pipeIndex === -1 ? rest : rest.slice(0, pipeIndex).trim();
  const styleRaw = pipeIndex === -1 ? null : rest.slice(pipeIndex + 1).trim();
...
function validateChartLine(
  ...
  const rest = line.slice(eqIndex + 1).trim();
  const pipeIndex = rest.indexOf("|");
  const typeRaw = (pipeIndex === -1 ? rest : rest.slice(0, pipeIndex)).trim().toLowerCase();

  if (!VALID_CHART_TYPES.has(typeRaw)) {
    warnings.push(
      `Line ${lineNum}: Unsupported chart type '${typeRaw}'. Supported types: ${Array.from(VALID_CHART_TYPES).join(", ")}`
    );
  }
}
...
function validateDataValidationLine(
  ...
  if (!VALID_VALIDATION_TYPES.has(typeRaw)) {
    warnings.push(
      `Line ${lineNum}: Unsupported validation type '${typeRaw}'. Supported: ${Array.from(VALID_VALIDATION_TYPES).join(", ")}`
    );
  }

  if (pipeIndex === -1 || !rest.includes("range:")) {
...
function validateConditionalFormattingLine(
  ...
  if (!VALID_CONDITIONAL_TYPES.has(typeRaw)) {
    warnings.push(
      `Line ${lineNum}: Unsupported conditional format type '${typeRaw}'. Supported: ${Array.from(VALID_CONDITIONAL_TYPES).join(", ")}`
    );
  }
